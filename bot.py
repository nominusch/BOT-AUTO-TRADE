...
# IMPORT & ENV
# =====================
import os, sys, time, threading, requests, pandas as pd, ta, logging, hashlib
...
BSC_API_KEY = os.getenv("BSC_API_KEY")  # BSCScan API
...
lock = threading.Lock()
bot_active = False
user_wallets = {}  # wallet tiap user
wallet_tokens = {}  # token yang sudah dideteksi per user

# =====================
# UTILS
# =====================
def send_msg(text, chat_id=CHAT_ID):
    try:
...
def log_and_msg(text, level="info"):
    send_msg(text)
    if level=="info": logging.info(text)
    elif level=="warn": logging.warning(text)
    elif level=="error": logging.error(text)
...
def trend_ai_filter(df_short, df_medium, df_long):
    def analyze(df):
        ...
        trend = "SIDE"
        if last["close"]>last["ema20"]>last["ema50"] and 50<last["rsi"]<70: trend="UP"
        elif last["close"]<last["ema20"]<last["ema50"] and 30<last["rsi"]<50: trend="DOWN"
        body = abs(last["close"]-last["open"]); candle_range = last["high"]-last["low"]
    ...
    if trend_short==trend_med==trend_long and ai_short and ai_med and ai_long: return trend_short
    return "SIDE"

def generate_signal(df_short):
    rsi = ta.momentum.rsi(df_short["close"], window=14).iloc[-1]
    last = df_short.iloc[-1]; prev = df_short.iloc[-2]
    if rsi<30: return "BUY"
    elif rsi>70: return "SELL"
    if last["close"]>prev["high"]: return "BUY"
    elif last["close"]<prev["low"]: return "SELL"
    return None

def calc_atr_sl_tp(df, side, entry_price):
    atr = ta.volatility.average_true_range(df["high"], df["low"], df["close"], window=14).iloc[-1]
    if side=="BUY": sl=entry_price-sl_multiplier*atr; tp=entry_price+tp_multiplier*atr
    else: sl=entry_price+sl_multiplier*atr; tp=entry_price-tp_multiplier*atr
    return sl, tp
...
def exit_position(symbol, reason=""):
    global daily_pnl
    try:
        with lock: pos=positions[symbol]
        price=float(client.futures_symbol_ticker(symbol=symbol)["price"])
...
def manage_position(symbol, df_long):
    try:
        ...
        if (pos["side"]=="BUY" and price<=pos["sl"]) or (pos["side"]=="SELL" and price>=pos["sl"]): exit_position(symbol,"STOP LOSS"); return
        if (pos["side"]=="BUY" and price>=pos["tp"]) or (pos["side"]=="SELL" and price<=pos["tp"]): exit_position(symbol,"TAKE PROFIT"); return

        with lock:
            if pos["side"]=="BUY":
                if price>pos["trail_price"]: pos["trail_price"]=price
                elif price<pos["trail_price"]*(1-trailing_pct): exit_position(symbol,"TRAILING STOP")
            elif pos["side"]=="SELL":
                if price<pos["trail_price"]: pos["trail_price"]=price
                elif price>pos["trail_price"]*(1+trailing_pct): exit_position(symbol,"TRAILING STOP")
    except Exception as e:
...
# =====================
# TOKEN CHECKER / PANCAKESWAP / BSCSCAN
# =====================
def check_token_bscscan(token_address):
    ...
    try:
        ...
        else: return {"holders":0,"total_supply":0,"verified":False}
    except: return {"holders":0,"total_supply":0,"verified":False}

def check_token_pancake(token_address):
    url=f"https://api.pancakeswap.info/api/v2/tokens/{token_address}"
    try:
        r=requests.get(url).json()
        if "data" in r: return float(r["data"].get("liquidity",0))
        return 0
    except: return 0

def get_tokens_in_wallet(wallet_address):
    url=f"https://api.bscscan.com/api?module=account&action=tokentx&address={wallet_address}&startblock=0&endblock=99999999&sort=asc&apikey={BSC_API_KEY}"
    try:
        r=requests.get(url).json()
        tokens={}
        if r["status"]=="1":
...
def check_new_tokens():
    while True:
        for user_id, wallet in user_wallets.items():
            try:
                current_tokens = get_tokens_in_wallet(wallet)
                prev_tokens = wallet_tokens.get(user_id, {})
                new_tokens = {addr:sym for addr,sym in current_tokens.items() if addr not in prev_tokens}
                for addr, sym in new_tokens.items():
                    bsc=check_token_bscscan(addr)
                    pancake=check_token_pancake(addr)
                    text=f"🆕 New token detected in wallet {wallet}\nSymbol: {sym}\nContract: {addr}\nVerified: {bsc['verified']}\nHolders: {bsc['holders']}\nTotal Supply: {bsc['total_supply']}\nLiquidity Pancake: {pancake}"
                    try: bot.send_message(chat_id=user_id, text=text)
                    except: logging.error(f"Gagal kirim notif token baru ke {user_id}")
                wallet_tokens[user_id] = current_tokens
            except Exception as e:
                logging.error(f"Error check_new_tokens {user_id}: {e}")
        time.sleep(60)
...
def start_command(update: Update, context: CallbackContext):
    global bot_active; bot_active=True; update.message.reply_text("✅ Bot trading aktif!")

def stop_command(update: Update, context: CallbackContext):
    global bot_active; bot_active=False; update.message.reply_text("⛔ Bot trading dihentikan!")

def positions_command(update: Update, context: CallbackContext):
    if not positions: update.message.reply_text("No active positions.")
    else:
...
def balance_command(update: Update, context: CallbackContext):
    balance=get_balance(); update.message.reply_text(f"💰 Saldo USDT: {balance:.2f}")

def set_wallet_command(update: Update, context: CallbackContext):
    if len(context.args)!=1: update.message.reply_text("Usage: /set_wallet <ADDRESS>"); return
...
def check_token_command(update: Update, context: CallbackContext):
    if len(context.args)!=1: update.message.reply_text("Usage: /check_token <TOKEN_ADDRESS>"); return
    token=context.args[0]; bsc=check_token_bscscan(token); pancake=check_token_pancake(token)
    text=f"Token {token}\nVerified: {bsc['verified']}\nHolders: {bsc['holders']}\nTotal Supply: {bsc['total_supply']}\nLiquidity Pancake: {pancake}"
    update.message.reply_text(text)

# =====================
# RUN TELEGRAM & THREADS
# =====================
